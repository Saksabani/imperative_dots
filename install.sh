...
# ==============================================================================
ARCH_PKGS=(
    "niri" "swayidle" "kitty" "cava" "zbar" "rofi-wayland" 
    ...
    "grim" "playerctl" "satty" "yq" "xdg-desktop-portal" "xdg-desktop-portal-gnome" "slurp" "mpvpaper"
    "wmctrl" "power-profiles-daemon" "easyeffects" "swayosd-git" "swaync" "nautilus" "lsp-plugins" "polkit-gnome"
...
CONFIG_FOLDERS=("cava" "niri" "kitty" "rofi" "matugen" "zsh" "swayosd")
...
SETTINGS_FILE="$TARGET_CONFIG_DIR/niri/settings.json"
...
ENV_TARGET_DIR="$TARGET_CONFIG_DIR/niri/scripts/quickshell/calendar"
OLD_ENV_IN_BACKUP="$BACKUP_DIR/niri/scripts/quickshell/calendar/.env"
...
NIRI_CONF="$TARGET_CONFIG_DIR/niri/config.kdl"
ZSH_RC="$HOME/.zshrc"
WP_QML="$TARGET_CONFIG_DIR/niri/scripts/quickshell/wallpaper/WallpaperPicker.qml"
WP_DIR="$TARGET_CONFIG_DIR/niri/scripts/quickshell/wallpaper"

# -> Desktop/Laptop Battery Adaptability <-
QS_BAT_DIR="$TARGET_CONFIG_DIR/niri/scripts/quickshell/battery"
REPO_BAT_DIR="$REPO_DIR/.config/niri/scripts/quickshell/battery"
...
if [ -f "$NIRI_CONF" ]; then
    ...
    echo -e "  -> Applying Keyboard configuration to config.kdl..."
    sed -i -E "s|^([[:space:]]*)layout \"[^\"]*\"|\1layout \"$KB_LAYOUTS\"|" "$NIRI_CONF"
    
    if [ -n "$KB_OPTIONS" ]; then
        sed -i -E "s|^([[:space:]]*)options \"[^\"]*\"|\1options \"$KB_OPTIONS\"|" "$NIRI_CONF"
    else
        sed -i -E "s|^([[:space:]]*)options \"[^\"]*\"|\1options \"\"|" "$NIRI_CONF"
    fi

    # ========================================================================
    # BULLETPROOF NIRI ENV INJECTION
    # ========================================================================
    echo -e "  -> Applying Environment Variables safely to config.kdl..."

    # Clean up ANY previous injections
    sed -i '/^\/\/ === DOTFILES AUTO-INJECTED ENV ===/,/^\/\/ === END DOTFILES ENV ===/d' "$NIRI_CONF"

    # Prepare the new block
    ENV_INJECT="// === DOTFILES AUTO-INJECTED ENV ===\n    XDG_PICTURES_DIR \"$USER_PICTURES_DIR\"\n    XDG_VIDEOS_DIR \"$USER_VIDEOS_DIR\"\n    WALLPAPER_DIR \"$WALLPAPER_DIR\"\n    SCRIPT_DIR \"$HOME/.config/niri/scripts\"\n    QT_QPA_PLATFORMTHEME \"qt6ct\""
    ...
    awk -v inject="$ENV_INJECT" '
        ...
        { print }
    ' "$NIRI_CONF" > "${NIRI_CONF}.tmp" && mv -f "${NIRI_CONF}.tmp" "$NIRI_CONF"
    # ========================================================================
else
    echo -e "${C_RED}Warning: config.kdl not found at $NIRI_CONF${RESET}"
fi
...
UPSTREAM_KEYBINDS_CONF="$REPO_DIR/.config/niri/config.kdl"
UPSTREAM_BINDS_JSON="[]"
...
if [ -d "$TARGET_CONFIG_DIR/niri/scripts" ]; then
    find "$TARGET_CONFIG_DIR/niri/scripts" -type f -exec sed -i -e 's/swww-daemon/swww-daemon/g' -e 's/swww/swww/g' {} +
fi
...
if [ -f "$ZSH_RC" ]; then
    ...
    echo "export SCRIPT_DIR=\"$HOME/.config/niri/scripts\"" >> "$ZSH_RC"
