...
# ---------------------------------------------------------
# MULTI-MONITOR GEOMETRY FIX (NIRI)
# ---------------------------------------------------------
...
if [ -n "$GEOMETRY" ]; then
    ...
    OFFSET=$(niri msg --json focused-output 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); pos=d.get('logical',d.get('position',{})); print(f\"{pos.get('x',0)} {pos.get('y',0)}\")" 2>/dev/null)
