...
# -----------------------------------------------------------------------------
# CONSTANTS & ARGUMENTS (Niri version)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# FAST PATH: WORKSPACE SWITCHING (Niri version)
# Replaces: hyprctl --batch "dispatch workspace N"
# -----------------------------------------------------------------------------
if [[ "$ACTION" =~ ^[0-9]+$ ]]; then
    ...
    if [[ "$2" == "move" ]]; then
        niri msg action move-window-to-workspace "$WORKSPACE_NUM" >/dev/null 2>&1
    else
...
# ZOMBIE WATCHDOG
# -----------------------------------------------------------------------------
MAIN_QML_PATH="$HOME/.config/niri/scripts/quickshell/Main.qml"
BAR_QML_PATH="$HOME/.config/niri/scripts/quickshell/TopBar.qml"
