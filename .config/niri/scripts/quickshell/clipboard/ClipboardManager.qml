...
Item {
    ...
    Process {
        id: clipFetcher
        running: true
        command: ["python3", Quickshell.env("HOME") + "/.config/niri/scripts/quickshell/clipboard/clip_fetcher.py", window.currentOffset, window.fetchLimit]
    ...
    function loadMore() {
        ...
        clipFetcher.command = ["python3", Quickshell.env("HOME") + "/.config/niri/scripts/quickshell/clipboard/clip_fetcher.py", window.currentOffset, window.fetchLimit];
    ...
    Connections {
        ...
        function onVisibleChanged() {
            if (window.visible) {
                ...
                clipFetcher.command = ["python3", Quickshell.env("HOME") + "/.config/niri/scripts/quickshell/clipboard/clip_fetcher.py", 0, window.fetchLimit];
                clipFetcher.running = true;
            } else {
                searchInput.text = "";
                window.pendingIndex = -1;
