...
def is_lockscreen_active():
    try:
        # lock.sh launches: quickshell -p ~/.config/niri/scripts/quickshell/Lock.qml
        subprocess.check_output(['pgrep', '-f', 'quickshell.*Lock.qml'], text=True)
        return True
...
def get_active_window_niri():
    try:
        output = subprocess.check_output(['niri', 'msg', '--json', 'focused-window'], text=True)
        if output.strip() in ("", "null", "{}"):
            return ("Locked", "Locked") if is_lockscreen_active() else ("Desktop", "Desktop")
        data = json.loads(output)
        if data is None:
            return ("Locked", "Locked") if is_lockscreen_active() else ("Desktop", "Desktop")

        app_cls = data.get('app_id') or data.get('class') or data.get('initialClass') or ''
