...
Item {
    ...
    function saveAppSettings() {
        ...
        let cmd = "mkdir -p ~/.config/niri/ && [ ! -f ~/.config/niri/settings.json ] && echo '{}' > ~/.config/niri/settings.json; " +
                  "jq '. + " + jsonString + "' ~/.config/niri/settings.json > ~/.config/niri/settings.json.tmp && " +
                  "mv ~/.config/niri/settings.json.tmp ~/.config/niri/settings.json && " +
                  "notify-send 'Quickshell' 'Settings Applied Successfully!'";
        Quickshell.execDetached(["bash", "-c", cmd]);
        if (root.setWorkspaceCount !== root.initialWorkspaceCount) {
            Quickshell.execDetached(["qs", "-p", Quickshell.env("HOME") + "/.config/niri/scripts/quickshell/TopBar.qml", "ipc", "call", "topbar", "queueReload"]);
    ...
    function saveWeatherConfig() {
        ...
        var file = Quickshell.env("HOME") + "/.config/niri/scripts/quickshell/calendar/.env";
        var cmds = [
    ...
    Process {
        id: envReader
        command: ["bash", "-c", "cat ~/.config/niri/scripts/quickshell/calendar/.env 2>/dev/null || echo ''"]
        running: true
    ...
    function normalizeDispatcher(d) {
        if (!d || d === "") return "spawn";
        const map = {
            "exec": "spawn",
            "exec-once": "spawn",
            ...
            "movefocus": "focus-column-right",
            "movewindow": "move-column-right",
            "resizeactive": "set-column-width",
    ...
    function saveAllKeybinds() {
        ...
        let cmd = "mkdir -p ~/.config/niri/ && [ ! -f ~/.config/niri/settings.json ] && echo '{}' > ~/.config/niri/settings.json; " +
                  "jq '.keybinds = " + jsonStr + "' ~/.config/niri/settings.json > ~/.config/niri/settings.json.tmp && " +
                  "mv ~/.config/niri/settings.json.tmp ~/.config/niri/settings.json && " +
    ...
    Process {
        id: niriLangReader
        command: ["bash", "-c", "grep -m1 '^ *layout *\"' ~/.config/niri/config.kdl | sed -E 's/^ *layout *\"([^\"]+)\".*/\\1/'"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                        ...
                        if (parsed.keybinds !== undefined && Array.isArray(parsed.keybinds)) {
                            ...
                            for (let k of parsed.keybinds) {
                                tempBinds.push({
                                    ...
                                    mods: k.mods || "",
                                    key: k.key || "",
                                    dispatcher: normalizeDispatcher(k.dispatcher || "spawn"),
                                    command: k.command || "",
                                    isEditing: false
                                });
                            }
    ...
    Process {
        ...
        command: ["bash", "-c", "q=\"$1\"; ls -dp -- \"${q}\"* 2>/dev/null | grep '/$' | head -n 5 || true", "_", query]
        stdout: StdioCollector {
            onStreamFinished: {
    ...
    property var allSettingsCards: [
        ...
        { tab: 0, boxIndex: 3, label: "Keyboard layouts",  desc: "Matches niri config",    icon: "󰌌", color: "green" },
    ...
    SequentialAnimation {
        ...
        ScriptAction { 
            script: {
                // Niri has no submap concept - no reset needed

                if (root.requiresReload) {
                    let script = Quickshell.env("HOME") + "/.config/niri/scripts/qs_manager.sh close; " +
                                 "while [ -n \"$(cat /tmp/qs_current_widget 2>/dev/null)\" ]; do sleep 0.1; done; " +
                                 "sleep 0.2; " + 
                                 "qs -p ~/.config/niri/scripts/quickshell/TopBar.qml ipc call topbar forceReload";
                        ...
                        ColumnLayout {
                            ...
                            RowLayout {
                                ...
                                ColumnLayout {
                                    ...
                                    Text {
                                        text: "Matches niri config.kdl. Click ✖ to remove."; font.family: "Inter"; font.pixelSize: root.s(11)
                            ...
                            ColumnLayout {
                                ...
                                Item {
                                    ...
                                    MouseArea {
                                        ...
                                        onClicked: {
                                            if (model.dispatcher === "spawn") {
                                                Quickshell.execDetached(["bash", "-c", model.command]);
                                            } else if (model.command && model.command.trim() !== "") {
                                                Quickshell.execDetached(["niri", "msg", "action", model.dispatcher, model.command]);
                                            } else {
                                                Quickshell.execDetached(["niri", "msg", "action", model.dispatcher]);
                                            }
                                        }
                                    }
                                }
                                ...
                                ColumnLayout {
                                    ...
                                    Rectangle {
                                        ...
                                        Item {
                                            ...
                                            onActiveFocusChanged: {
                                                if (!activeFocus) { accumulatedMods = []; accumulatedKey = ""; }
                                            }
                                        }
                                    }
                ...
                RowLayout {
                    ...
                    Rectangle {
                        ...
                        MouseArea {
                            ...
                            cursorShape: Qt.PointingHandCursor
                            onClicked: {
                                dynamicKeybindsModel.append({ type: "bind", mods: "", key: "", dispatcher: "spawn", command: "", isEditing: true });
                                scrollTimer.start();
                            }
                        }
                    }
