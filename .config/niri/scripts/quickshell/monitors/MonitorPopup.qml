...
Item {
    ...
    function triggerApply() {
        ...
        if (monitorsModel.count === 1) {
            ...
            Quickshell.execDetached(["sh", "-c", "notify-send 'Display Update' 'Saved to settings.json. Restart Niri to apply.' ; " + jsonCmd + " ; " + postReloadCmd]);
            ...
            let applyCmd = jsonCmd + " ; " + postReloadCmd;

            Quickshell.execDetached(["sh", "-c", applyCmd]);
            Quickshell.execDetached(["notify-send", "Display Update", "Saved layout for: " + summaryString + ". Restart Niri to apply."]);
            
            window.debugLog("Executed multi monitor apply (saved to settings.json).");
        }
    }
